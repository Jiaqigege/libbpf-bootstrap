...
int main(int argc, char **argv)
{
	...
	prog_fd = load_bpf_object_file__simple(".output/get_pkts_legacy_mode.bpf.o");
	if (prog_fd <= 0) {
		fprintf(stderr, "ERR: loading file: %s\n", "get_pkts_legacy_mode.o");
		return -1;
	}
	...
	if (attach) {
		...
		if (err < 0) {
			...
			case EOPNOTSUPP:
				fprintf(stderr,
					"Native-XDP not supported on the device %s, try using --skb-mode\n",
					argv[1]);
				break;
			default:
				break;
