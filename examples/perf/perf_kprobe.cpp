// refer to https://github.com/zq-david-wang/linux-tools/blob/main/perf/kprobe/perf_kprobe.cpp
...
//
//
static long perf_event_open(struct perf_event_attr *perf_event, pid_t pid, int cpu, int group_fd,
			    unsigned long flags)
{
...
int process_event(char *base, unsigned long long size, unsigned long long offset)
{
	...
	if (p->type == PERF_RECORD_SAMPLE) {
		...
		abi = *(unsigned long long *)(base + offset);
		offset += 8;
		if (offset >= size)
			offset -= size;
		if (abi == PERF_SAMPLE_REGS_ABI_64) {
			...
			if (offset >= size)
				offset -= size; //rax
			arg3 = *(unsigned long long *)(base + offset);
			offset += 8;
			if (offset >= size)
				offset -= size; //rdx
			arg2 = *(unsigned long long *)(base + offset);
			offset += 8;
			if (offset >= size)
				offset -= size; //rsi
			arg1 = *(unsigned long long *)(base + offset);
			offset += 8;
			if (offset >= size)
				offset -= size; //rdi
			arg4 = *(unsigned long long *)(base + offset);
			offset += 8;
			if (offset >= size)
				offset -= size; //r10
			printf("%s@%s[%d] %s(0x%llx,0x%llx,0x%llx,0x%llx)\n",
			       get<1>(pids[pid]).c_str(), get<2>(pids[pid]).c_str(), pid, func,
			       arg1, arg2, arg3, arg4);
		}
	}
	return p->size;
...
int main(int argc, char *argv[])
{
	...
	// start perf event
	if (argc < 2) {
		printf("Need kprobe function name, e.g. %s do_sys_open\n", argv[0]);
		return 1;
	}
	...
	attr.probe_offset = 0;
	attr.sample_regs_intr = (1 << PERF_REG_X86_AX) | (1 << PERF_REG_X86_DI) |
				(1 << PERF_REG_X86_SI) | (1 << PERF_REG_X86_DX) |
				(1 << PERF_REG_X86_R10) | (1 << PERF_REG_X86_R8) |
				(1 << PERF_REG_X86_R9);
	int fd;
	...
	while (poll(polls, k, -1) > 0) {
		for (i = 0; i < k; i++) {
			...
			while (head < mp->data_head)
				head += process_event((char *)addr + mp->data_offset, mp->data_size,
						      head);
